...
In this blog post, we'll walk through the process of deploying a Shiny app to production using Google Cloud Platform (GCP).

In this post, we'll walk you through the process of deploying a Shiny app to production on Google Cloud Platform (GCP). We'll use a simple dockerized Shiny app called `everysinglecountry`, which interacts with public APIs. This app doesn't require a _backend_, _database_, or _authentication_, as we'll cover those topics in future tutorials. If you're looking to expand your app with these features, stay tuned!
